...
const showConfirm = ref(false) // confirm the passwords are matching
const rules = {
    required: value => !!value ||'Required.', // if statement
    min: v=> v.length >= 8 || 'Min 8 characters',
     passwordMatch: () => password.value == coonfirmPassword.value || 'Passwords must match'
}
...
function handleSignup()
{
    const data = {
        ...
        role: 2, //role 1 is for admin, role 2 is for patient
    }
    signup(data)
    router.push('/homepage').then(() => { //after loginin your taken to homepage
        router.go(0)
    });
}
</script>
